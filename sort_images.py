...
def parse_filename(filename):
   ...
   if match:
       ...
       return {
           'date': date_str,
           'time': time_str,
           'sequence': int(sequence_str),  # This removes AATP prefix, just stores the number
           'timestamp': (date_str, time_str, sequence_str)
       }
   return None

def main():
   parser = argparse.ArgumentParser(description='Sort image files by timestamp or sequence and output in ffmpeg list format')
   parser.add_argument('directory', help='Directory containing image files')
   parser.add_argument('-o', '--output', default='ffmpeg_list.txt', help='Output file (default: ffmpeg_list.txt)')
   parser.add_argument('-s', '--sort-by', choices=['timestamp', 'sequence'], default='sequence', 
   ...
   print(f"Generated {args.output} with {len(jpg_files)} files sorted by {sort_method}")
