        ...
        if file_path.lower().endswith('.json'):
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError("File format not recognized. Please use .json or .yaml/.yml extension")
...
def calculate_duration_differences(grouped_data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Dict[str, Any]]:
...
def display_results(results: Dict[str, Dict[str, Any]], console: Console):
    ...
    sorted_results = sorted(results.items(), key=lambda x: x[0] if x[0] != 'unknown' else 'zzz')

    for ratio, data in sorted_results:
        range_diff = data['max_duration_seconds'] - data['min_duration_seconds']
        range_human = parse_duration_seconds(int(range_diff)) if range_diff > 0 else "0m"
...
def display_detailed_results(results: Dict[str, Dict[str, Any]], console: Console):
    ...
    for ratio, data in sorted(results.items()):
        ...
        for video in data['videos']:
            notes = video.get('notes', '')[:50] + "..." if len(video.get('notes', '')) > 50 else video.get('notes', '')
...
def main():
    parser = argparse.ArgumentParser(description='Analyze sourdough starter time-lapse data')
    parser.add_argument('file_path', help='Path to JSON or YAML manifest file')
    parser.add_argument('--detailed', '-d', action='store_true',
                       help='Show detailed results for each ratio')
    parser.add_argument('--sort-by', choices=['ratio', 'duration', 'count'],
                       default='ratio', help='Sort results by specified field')
    ...
    try:
        ...
        # Display results
        console.print(f"[bold]Analyzing {len(videos)} video records from {args.file_path}[/bold]\n")
